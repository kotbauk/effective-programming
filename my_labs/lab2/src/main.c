...
inline my_vec_t __attribute__((__always_inline__))
get_left_vector(my_vec_t a, my_vec_t b) {
  return _mm256_permute4x64_pd(_mm256_blend_pd((a), (b), 0x07), 0b10010011);
}

/* (a, b, c, d); (e, f, g, h) -> (b, c, d, e) */
inline my_vec_t __attribute__((__always_inline__))
get_right_vector(my_vec_t a, my_vec_t b) {
...
inline my_float_t compute_po_in_point(my_float_t xj,
                                      my_float_t yi,
                                      my_float_t xs1,
                                      my_float_t xs2,
                                      my_float_t ys1,
...
my_matrix_t* compute_results(my_matrix_t* results_matrix,
  ...
  for (size_t n = 0; n < number_of_steps; ++n) {
    for (size_t i = 1; i < mod_area.number_y_points - 1; ++i) {
      ...
      for (size_t j = 0; j < mod_area.number_x_points; j += SCAL_IN_VEC) {
        my_vec_t vec_po_left = get_left_vector(vec_po_prev, vec_po_cur);
        my_vec_t vec_po_right = get_right_vector(vec_po_cur, vec_po_next);

        my_vec_t vec_res_left = get_left_vector(vec_res_prev, vec_res_cur);
        my_vec_t vec_res_right = get_right_vector(vec_res_cur, vec_res_next);
        ...
        my_vec_t po_val = _mm256_fmadd_pd(
            ...
            _mm256_mul_pd(
                vec_quarter,
                _mm256_add_pd(_mm256_add_pd(vec_po_low, vec_po_up),
